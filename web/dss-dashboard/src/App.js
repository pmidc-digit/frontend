...
let dataL = {
  en: {
    ...
    "DSS_TOTAL_CUMULATIVE_COLLECTION:_DEPARTMENT_WISE":
    ...
    "DSS_TOTAL_APPLICATION_&_CLOSED_APPLICATION":
      "Total Applications & Closed Applications",
    "DSS_TOTAL_APPLICATIONS:_DEPARTMENT_WISE":
    ...
    DSS_PT_CUMULATIVE_PROPERTIES_ASSESSED:
...
class App extends React.Component {
  ...
  loadLocalisation = () => {
    ...
    let tenent = localStorage.getItem("tenant-id")
      ...
      : "pb";
    let localisationLabels =
      JSON.parse(localStorage.getItem(`localization_${language}`)) || [];
    if (
      localisationLabels.length == 0 ||
      localisationLabels.filter(
        (localisation) => localisation.module == "rainmaker-dss"
      ).length == 0
    ) {
      let localisationRequest = fetchLocalisationRequest(language, tenent);
      axios
        .post(
          localisationRequest.reqUrl,
          localisationRequest.reqBody,
          localisationRequest.reqHeaders
        )
        .then((response) => {
  ...
  setLocalisation = (localisationLabels = []) => {
    let data = _.chain(localisationLabels)
      .map((i) => {
        return { key: i.code, value: i.message };
      })
      .flatten()
      .value();
    let newIndex = _.chain(data).keyBy("key").mapValues("value").value();
    let dataL = {
      en: newIndex,
      hi: {},
    };
