...
class GlobalFilter extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      role:
        this.props.dashboardConfigData &&
        Array.isArray(this.props.dashboardConfigData) &&
        this.props.dashboardConfigData.length > 0 &&
  ...
  componentDidMount() {
    let tenentCode = localStorage.getItem("tenant-id")
  ...
  componentDidUpdate(prevProps) {
    if (prevProps.tenents !== this.props.tenents) {
      ...
      let tenentObj = {};
      if (
        this.state.tenentList &&
        Array.isArray(this.state.tenentList) &&
        this.state.tenentList.length > 0
      ) {
    ...
    if (prevProps.wards !== this.props.wards) {
      let tenantsBoundry = _.get(
        this.props.wards,
        "MdmsRes.egov-location.TenantBoundary"
      );
      ...
      if (
        tenantsBoundry &&
        Array.isArray(tenantsBoundry) &&
        tenantsBoundry.length > 0
      ) {
        tenantsBoundry.map((boundaryData) => {
          if (
            boundaryData["hierarchyType"] &&
            boundaryData["hierarchyType"].code &&
            boundaryData["hierarchyType"].code === "REVENUE"
          ) {
            let tenantsBoundryObj = boundaryData["boundary"];

            if (
              tenantsBoundryObj &&
              tenantsBoundryObj.children &&
              Array.isArray(tenantsBoundryObj.children) &&
              tenantsBoundryObj.children.length > 0
            ) {
              tenantsBoundryObj.children.map((tenantsBoundryObjChildren) => {
                if (
                  tenantsBoundryObjChildren &&
                  tenantsBoundryObjChildren.children &&
                  Array.isArray(tenantsBoundryObjChildren.children) &&
                  tenantsBoundryObjChildren.children.length > 0
                ) {
    ...
    if (target) {
      ...
      if (target !== "duration") {
        ...
        if (newFilterData.duration.title === "CUSTOM") {
          ...
          newFilterData.duration.title =
            moment
              .unix(_.get(newFilterData, "duration.value.startDate"))
              .format(formatYearlessL) +
            " - " +
            moment
              .unix(_.get(newFilterData, "duration.value.endDate"))
              .format(formatYearlessL);
        }
      ...
      <FormControl fullWidth className={classes.formControl}>
        <div className={classes.list}>
          ...
          <Input
            id="adornment-amount"
            value={
              _.get(this.state, "filterData.duration.title") || this.state.value
            }
            className={classes.dateFilter}
            endAdornment={
              <DropDownIcon style={{ color: "#656565" }}></DropDownIcon>
            }
  ...
  renderSimpleSelect(label, data) {
    ...
    return (
      <SimpleSelect
        ...
        target={label}
        selected={this.state[label] || `All ${label}`}
  ...
  renderSwitch(label, data) {
    ...
    return (
      <SwitchButton
        ...
        padding="3px"
        fontSize="14px"
        selected={this.state.filterData[label] || data[0]}
        target={label}
        data={newData}
        handleSelected={this.handleChanges}
      ></SwitchButton>
    );
  }
              ...
              {GFilterData.Districts.map((item) => {
                ...
                return (
                  <div style={{ margin: isMobile ? "4px 0 0 0" : "0 4px 0 0" }}>
                    <Chip
                      fullwidth
                      className={classes.mCustomChip}
                      label={item}
                      style={{ padingLeft: "3px" }}
                      color={"gray"}
                    ></Chip>
                  </div>
                );
            ...
            <div className={classes.mainFilter}>
              ...
              {isMobile ? (
                <div
                  ...
                  style={{ maxWidth: "inherit", marginTop: "7px" }}
                >
                  <ActionButtons
                    buttonType="default"
                    fontSize="16px"
                    type="clearButton"
                    text={strings["DSS_CLEAR_ALL"] || "DSS_CLEAR_ALL"}
                    disableed={Object.keys(this.state.filterData).length == 0}
                    clas={classes.clearbtn}
                    handleClick={this.clearFilter.bind(this)}
                  />
                  <ActionButtons
                    containedButton={true}
                    buttonType="default"
                    fontSize="16px"
                    text={strings["DSS_APPLY"] || "DSS_APPLY"}
                    clas={classes.applybtn}
                    handleClick={this.applyFilter.bind(this)}
                  />
                </div>
              ) : (
                <div
                  id="divNotToPrint"
                  className={classes.actions}
                  style={{ maxWidth: 300, marginTop: "7px" }}
                >
                  <ActionButtons
                    buttonType="default"
                    fontSize="16px"
                ...
                {ulbOverViewFilters.map((ro) => {
                  return (
                    <div
                      ...
                      className={`${classes.filterS} ${"GF_" + ro.label}`}
                    >
                      {this.state.pageId === "ulb-overview" &&
                      ro.label === "Wards" ? (
                        <div></div>
                      ) : (
                        <div className={classes.filterHead}>
                      ...
                      <div>{this.renderUlbFilters(ro)}</div>
                    </div>
                  );
                })}
...
export default withRouter(
  withStyles(styles)(connect(mapStateToProps, mapDispatchToProps)(GlobalFilter))
);
