...
import { Taskboard } from "../Inbox/components/actionItems";
import FilteredFilter from "./FilteredFilter";
import InboxData from "../Inbox/components/Table";
import "../Inbox/components/TableData/index.css";
...
class TableData extends Component {
  ...
  /**
   * Check if businessService matches module pattern
   * Module "PT" matches: "PT", "PT.CREATE", "PT.MUTATION"
   * Module "TL" matches: "NewTL", "ModifyTL", etc.
   */
  matchesModulePattern = (businessService, modulePattern) => {
    // Exact match
    if (businessService === modulePattern) {
      return true;
    }
    // Prefix match with dot: PT matches PT.CREATE
    if (businessService.startsWith(modulePattern + '.')) {
      return true;
    }
    // Contains pattern for New/Modify: TL matches NewTL, ModifyTL
    if (businessService.includes(modulePattern) &&
        (businessService.startsWith('New') || businessService.startsWith('Modify'))) {
      return true;
    }
    return false;
  }
  ...
  checkRow = (row, filter, searchFilter, taskboardLabel) => {
    const businessService = row[2].text.props.label.split('_')[1];

    // If we have preselectedModule (from URL), skip module matching check
    // because we already filtered at API level by businessServices
    const { preselectedModule } = this.props;
    const moduleMatches = preselectedModule
      ? true  // Always pass - data is already filtered by API
      : (filter.moduleFilter.selectedValue.includes('ALL') ||
         filter.moduleFilter.selectedValue.some(module => this.matchesModulePattern(businessService, module)));

    // Business Service filter - filter on frontend based on dropdown selection
    const businessServiceMatches = filter.businessServiceFilter.selectedValue.includes('ALL') ||
      filter.businessServiceFilter.selectedValue.includes(row[0].subtext);

    if ((filter.localityFilter.selectedValue.includes('ALL') || filter.localityFilter.selectedValue.includes(row[1].text.props.label)) &&
      moduleMatches &&
      businessServiceMatches &&
...
if(totalRows.length == totalRowCount && showLoadingTaskboard==false){
  ...
  clearFilter = () => {
    ...
    const filter = {
      ...
      moduleFilter: {
        // Keep preselected module if it exists, otherwise clear to ALL
        selectedValue: preselectedModule ? [preselectedModule] : ["ALL"],
        dropdownData: [...this.state.filter.moduleFilter.dropdownData]
      },
      businessServiceFilter: {
        selectedValue: ["ALL"],
        dropdownData: [...this.state.filter.businessServiceFilter.dropdownData]
      },
      statusFilter: {
        selectedValue: ["ALL"],
  ...
  prepareInboxDataRows = async (data, all, loadLocality = false) => {
    const { toggleSnackbarAndSetText, businessServices: configuredBusinessServices } = this.props;
    ...
    let businessServiceDropdownList = [];
    ...
    const initialData = data.map((item) => {
      ...
      let businessServiceDropdown = { label: item.businessService, value: item.businessService };
      businessServiceDropdownList.push(businessServiceDropdown);
    ...
    if (all) {
      ...
      // Build business service dropdown data
      let businessServiceDropdownData = [];

      // If we have configured business services (from module config), use those
      if (configuredBusinessServices && configuredBusinessServices.length > 0) {
        businessServiceDropdownData = configuredBusinessServices.map(service => ({
          // Use localization key pattern: CS_COMMON_INBOX_{SERVICE}
          // e.g., CS_COMMON_INBOX_PT.CREATE, CS_COMMON_INBOX_PT.MUTATION
          label: getLocaleLabels("", `CS_COMMON_INBOX_${service}`, localizationLabels),
          value: service
        }));
      } else {
        // Otherwise, use business services found in the data
        businessServiceDropdownData = businessServiceDropdownList;
      }
  ...
  loadInitialData = async () => {
    const { toggleSnackbarAndSetText, prepareFinalObject, preselectedModule, businessServices } = this.props;
    ...
    try {
      ...
      // Make multiple API calls if businessServices array exists
      // Fetch ALL data from each businessService (no pagination on backend)
      if (businessServices && businessServices.length > 0) {
        // Fetch data for each businessService separately - get ALL records
        for (const service of businessServices) {
          const requestBody = [
            { key: "tenantId", value: tenantId },
            // No limit/offset - fetch all data from backend for this businessService
            { key: "businessService", value: service }
          ];

          try {
            const responseData = await httpRequest("egov-workflow-v2/egov-wf/process/_search", "_search", requestBody);
            const processInstances = get(responseData, "ProcessInstances", []);
            allProcessInstances = [...allProcessInstances, ...processInstances];
          } catch (error) {
            console.error(`[ModuleInbox] Error fetching data for businessService: ${service}`, error);
            // Continue with other services even if one fails
          }
        }
      } else {
        // No module filter - fetch all data
        const requestBody = [
          { key: "tenantId", value: tenantId }
          // No limit/offset - get all workflow data
        ];
        const responseData = await httpRequest("egov-workflow-v2/egov-wf/process/_search", "_search", requestBody);
        allProcessInstances = get(responseData, "ProcessInstances", []);
      }

      // Remove duplicates (in case same instance appears in multiple businessServices)
      const uniqueInstances = allProcessInstances.filter((instance, index, self) =>
        index === self.findIndex((t) => t.id === instance.id)
      );
      const allData = orderBy(uniqueInstances, ["businesssServiceSla"]);
      // Load locality data in background (async - don't wait)
      ...
      this.setState({
        loaded: true,
        totalRowCount: allDataRows.length, // Total count is the actual data we have
        inboxData, taskboardData, tabData, initialInboxData: cloneDeep(inboxData)
      });
      this.hideLoading()
    } catch (e) {
      console.error('[ModuleInbox] Error in loadInitialData:', e);
      console.error('[ModuleInbox] Error message:', e.message);
      console.error('[ModuleInbox] Error stack:', e.stack);
      this.hideLoading();
  ...
  loadRemainingData = async (offset, limit, businessServices, previousResponse) => {
    const { toggleSnackbarAndSetText, prepareFinalObject } = this.props;
    const tenantId = getTenantId();
    let { taskboardData, tabData } = this.state;
    const inboxData = [{ headers: [], rows: [] }];
    try {
      let allProcessInstances = [...get(previousResponse, "ProcessInstances", [])];

      // Make multiple API calls for pagination if businessServices array exists
      if (businessServices && businessServices.length > 0) {
        for (const service of businessServices) {
          const requestBody = [
            { key: "tenantId", value: tenantId },
          ...
          try {
            ...
            console.error(`Error fetching remaining data for businessService: ${service}`, error);
            // Continue with other services even if one fails
          }
        }
      } else {
  ...
  loadLocalityForAllData = async (allData) => {
    ...
    try {
      ...
      console.error('[ModuleInbox] loadLocalityForAllData: Error occurred', e);
      console.error('[ModuleInbox] loadLocalityForAllData: Error message:', e.message);
      this.hideLoading();
      // Don't show error toast here - locality loading failure shouldn't block the whole inbox
      // toggleSnackbarAndSetText(true, { labelName: "Workflow search error !", labelKey: "ERR_SEARCH_ERROR" }, "error");
      console.warn('[ModuleInbox] loadLocalityForAllData: Continuing without locality data');
    }
    prepareFinalObject("InboxData", [...inboxData]);
    this.getMaxSLA();
  }
  ...
  render() {
    ...
    const { classes, preselectedModule } = this.props;
      ...
      <div className="col-md-12 col-sm-12 col-xs-12">
        <div>
          ...
          <Hidden only={["xs"]} implementation="css">
            <FilteredFilter handleChangeFilter={handleChangeFilter.bind(this)} clearFilter={clearFilter} filter={filter} preselectedModule={preselectedModule}></FilteredFilter></Hidden>
          <Hidden only={["sm", "md", "lg", "xl"]} implementation="css">
            {this.state.showFilter &&
              <FilteredFilter handleChangeFilter={handleChangeFilter.bind(this)} clearFilter={clearFilter} filter={filter} preselectedModule={preselectedModule}></FilteredFilter>}
          </Hidden>
        </div>
