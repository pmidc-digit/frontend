...
import { getRequestBody, getTenantName } from "./publicSearchUtils";
import { validateFields } from "../../utils/index";
import { convertEpochToDate } from "../../utils/index";
import {
  findAndReplace,
  getOpenSearchResultsForWater,
  getOpenSearchResultsForSewerage,
  getWorkFlowData,
  serviceConst,
  getMdmsDataForBill,
...
export const getPayload = (searchScreenObject) => {
  ...
  if (searchScreenObject) {
    ...
    if (searchScreenObject.ids) {
      queryObject.push({ key: "propertyId", value: searchScreenObject.ids });
    }
...
const searchApiCall = async (state, dispatch) => {
  ...
  if (
    ...
    try {
      ...
      // Call Water Service API if Water is selected
      if (searchScreenObject.transactionType === "Water") {
      ...
      // Call Sewerage Service API if Sewerage is selected
      else if (searchScreenObject.transactionType === "Sewerage") {
...
const showResults = (connections, tenantId, dispatch) => {
  ...
  let data = connections.map((item) => ({
    ...
    ["WS_COMMON_TABLE_COL_CONSUMER_NO_LABEL"]: item.connectionNo
      ? item.connectionNo
      : "NA",
    ["WS_COMMON_TABLE_COL_OWN_NAME_LABEL"]: get(
      item,
    ...
    ["WS_COMMON_TABLE_COL_DUE_LABEL"]:
      item.totalAmount || item.totalAmount === 0 ? item.totalAmount : "NA",
    ["WS_COMMON_TABLE_COL_ADDRESS"]: getAddress(get(item, "property", {})),
    ["WS_COMMON_TABLE_COL_DUE_DATE_LABEL"]:
      item.updatedDueDate !== undefined
        ? convertEpochToDate(item.updatedDueDate)
        : "NA",
...
export const resetFields = (state, dispatch) => {
  let ulbCityValue = get(
    state,
    "screenConfiguration.preparedFinalObject.searchScreen.tenantId",
    ""
  );
  let localityValue = get(
    state,
    "screenConfiguration.preparedFinalObject.searchScreen.locality.code",
    ""
  );
  if (ulbCityValue) {
    dispatch(
