...
import {
  convertDateToEpoch,
  convertEpochToDate,
  resetFieldsForApplication,
  resetFieldsForConnection,
} from "../../utils/index";
...
export const searchApiCall = async (state, dispatch) => {
  ...
  let getCurrentTab = get(
    state.screenConfiguration.preparedFinalObject,
    "currentTab"
  );
  let currentSearchTab =
    getCurrentTab === undefined ? "SEARCH_CONNECTION" : getCurrentTab;
...
const renderSearchConnectionTable = async (state, dispatch) => {
  ...
  Object.keys(searchScreenObject).forEach(
    (key) => searchScreenObject[key] == "" && delete searchScreenObject[key]
  );
  if (Object.values(searchScreenObject).length <= 1) {
    dispatch(
      toggleSnackbar(
        ...
        {
          labelName:
            "Please provide the city and any one other field information to search for property.",
          labelKey: "ERR_PT_COMMON_FILL_MANDATORY_FIELDS",
        },
        "warning"
      )
    );
   ...
   searchScreenObject["ownerName"] && 
    ...
    try {
      ...
      try {
        ...
        payloadbillingPeriod = await httpRequest(
          "post",
      ...
      let getSearchResultForSewerage = getSearchResultsForSewerage(
        queryObject,
        dispatch
      );
      let finalArray = [];
      ...
      try {
        searchWaterConnectionResults = await getSearchResult;
      } catch (error) {
        finalArray = [];
        console.log(error);
      }
      try {
        searcSewerageConnectionResults = await getSearchResultForSewerage;
      } catch (error) {
        finalArray = [];
        console.log(error);
      }
      ...
      for (let i = 0; i < combinedSearchResults.length; i++) {
        ...
        if (element.connectionNo !== "NA" && element.connectionNo !== null) {
          ...
          if (element.service === serviceConst.WATER) {
            queryObjectForWaterFetchBill = [
              { key: "tenantId", value: searchScreenObject.tenantId },
          ...
          if (
            element.service === serviceConst.WATER &&
            ...
            payloadbillingPeriod.MdmsRes[
              "ws-services-masters"
            ].billingPeriod.forEach((obj) => {
          ...
          if (
            element.service === serviceConst.SEWERAGE &&
            payloadbillingPeriod &&
          ...
          let billResults = await fetchBill(
            queryObjectForWaterFetchBill,
            dispatch
          );
          ...
          billResults &&
            billResults.Bill.length > 0 &&
            billResults.Bill[0].billDetails.map((bill) => {
              if (element.service === serviceConst.WATER) {
...
const renderSearchApplicationTable = async (state, dispatch) => {
  ...
  if (!(isSearchBoxFirstRowValid && isSearchBoxSecondRowValid)) {
    ...
    for (var key in searchScreenObject) {
      if (
        ...
        if (key === "fromDate") {
          ...
          queryObject.push({
            key: key,
            value: searchScreenObject[key].replace(/ /g, "_"),
          });
        } else {
          queryObject.push({ key: key, value: searchScreenObject[key].trim() });
        }
    ...
    try {
      ...
      if (
        searchScreenObject.applicationType &&
        searchScreenObject.applicationType.toLowerCase().includes("water")
      ) {
        getSearchResult = getSearchResults(queryObject);
      } else if (
        searchScreenObject.applicationType &&
        searchScreenObject.applicationType.toLowerCase().includes("sewerage")
      ) {
        getSearchResultForSewerage = getSearchResultsForSewerage(
          queryObject,
          dispatch
        );
      } else {
      ...
      for (let i = 0; i < combinedSearchResults.length; i++) {
        ...
        if (
          element.applicationNo !== "NA" &&
          element.applicationNo !== undefined
        ) {
        ...
        if (i % 50 === 0 || i === combinedSearchResults.length - 1) {
          ...
          if (wfResponse !== null && wfResponse.ProcessInstances !== null) {
            combinedWFSearchResults = combinedWFSearchResults.concat(
              wfResponse.ProcessInstances
            );
          }
          appNo = "";
        }
      ...
      for (let i = 0; i < combinedSearchResults.length; i++) {
        ...
        if (
          ...
          if (
            ...
            let ownerName = "";
            element.property.owners.forEach((ele) => {
...
const showConnectionResults = (connections, dispatch) => {
  let data = connections.map((item) => ({
    ...
    ["WS_COMMON_TABLE_COL_DUE_DATE_LABEL"]:
      item.dueDate !== undefined && item.dueDate !== "NA"
        ? convertEpochToDate(item.dueDate)
        : item.dueDate,
    ...
    ["WS_COMMON_TABLE_COL_DISCHARGE_CONNECTION"]: item.dischargeConnection,
    ["WS_COMMON_TABLE_COL_DISCHARGE_FEE"]: item.dischargeFee,
  }));
  dispatch(
    handleField(
      "search",
...
const showApplicationResults = (connections, dispatch) => {
  let data = connections.map((item) => ({
    ...
    ["WS_COMMON_TABLE_COL_APP_TYPE_LABEL"]: getApplicationType(
      item.applicationType
    ),
    ["WS_COMMON_TABLE_COL_OWN_NAME_LABEL"]: item.name,
    ["WS_COMMON_TABLE_COL_APPLICATION_STATUS_LABEL"]: item.applicationStatus
      .split("_")
      .join(" "),
    ["WS_COMMON_TABLE_COL_ADDRESS"]: item.address,
    ["WS_COMMON_TABLE_COL_TENANTID_LABEL"]: item.tenantId,
    ...
    ["WS_COMMON_TABLE_COL_APPLICATION_DISCHARGE_CONNECTION"]:
      item.dischargeConnection,
    ["WS_HOME_SEARCH_RESULTS_OWN_MOB_LABEL"]: item.mobile,
  }));
  exceldata = data;
